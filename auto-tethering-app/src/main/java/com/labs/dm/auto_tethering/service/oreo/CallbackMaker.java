...
final class CallbackMaker {
    ...
    public CallbackMaker(Context c, OnStartTetheringCallback callback) {
        mContext = c;
        ...
        TypeId<OnStartTetheringCallback> t = TypeId.get(OnStartTetheringCallback.class);
    ...
    public void generateConstructor(TypeId<?> declaringType, TypeId<?> superType, FieldId<?, ?> appCallbackFieldId) {
        final MethodId<?, ?> superConstructor = superType.getConstructor();
        TypeId<?> myCallbackTypeId = TypeId.get(OnStartTetheringCallback.class);
